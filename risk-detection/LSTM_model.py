...
# Preprocess Data
reddit_data = reddit_data[['text', 'class']].rename(columns={"class": "label"})
twitter_data = twitter_data.rename(columns={"Tweet": "text", "Suicide": "label"})
reddit_data['label'] = reddit_data['label'].map({"suicide": 1, "non-suicide": 0})
twitter_data['label'] = twitter_data['label'].map({"Potential Suicide post ": 1, "Not Suicide post": 0})
