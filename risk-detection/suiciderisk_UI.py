        ...
        if lang != 'en':
            try:
                ...
                if translated and hasattr(translated, 'text'):
                    ...
                    # Fallback if translation object is malformed
                    return text, lang
            except Exception as translation_error:
                print(f"Translation service error: {translation_error}")
                return text, lang  # Return original text but keep detected language
...
def predict_text(text):
    try:
        ...
        return {"risk_score": 0.0, "risk_category": "🔴 Error in Analysis", "translated_text": text, "original_language": None}
...
if st.button("🔍 Analyze Risk"):
    if user_input.strip():
        ...
        if "Error in Analysis" in result['risk_category']:
