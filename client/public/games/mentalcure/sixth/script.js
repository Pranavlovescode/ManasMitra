...
function showResults() {
  ...
  try {
    const avgReactionTime = reactionTimes.length ? (reactionTimes.reduce((a, b) => a + b, 0) / reactionTimes.length) : 0;
    const payload = {
      score,
    ...
    window.parent && window.parent.postMessage({ type: 'mentalcure:result', gameId: 'sixth', payload }, '*');
  } catch (e) {
