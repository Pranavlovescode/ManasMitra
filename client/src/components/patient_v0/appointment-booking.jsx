...
export default function AppointmentBooking({ userId }) {
  ...
  const [appointments, setAppointments] = useState([]); // Type <any[]> removed
  const [showBooking, setShowBooking] = useState(false);
  const [selectedDate, setSelectedDate] = useState("");
  const [selectedTime, setSelectedTime] = useState("");
  const [sessionType, setSessionType] = useState("individual"); // Type union removed
  ...
  const handleBookAppointment = async () => {
    if (!selectedDate || !selectedTime) {
      alert("Please select date and time"); // Note: 'alert' might be blocked in some environments.
      return;
    }
      ...
      {!showBooking ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Schedule a therapy session with your therapist
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) => setSessionType(e.target.value)} // 'as' assertion removed
                  />
