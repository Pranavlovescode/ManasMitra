...
import { computeGameAnalytics, buildGameInsights } from "@/lib/analysis";
...
export async function GET(request, { params }) {
  try {
    ...
    const patient = await Patient.findById(patientId).select(
      "clerkId medicalInfo.assignedTherapist"
    );
    if (!patient) {
    ...
    if (!assigned || assigned !== therapist._id.toString()) {
      return NextResponse.json(
        { error: "Forbidden: patient not assigned to therapist" },
        { status: 403 }
      );
    }
  ...
  const insights = buildGameInsights(results);

  return NextResponse.json({ patientId, ...analytics, insights });
  } catch (error) {
