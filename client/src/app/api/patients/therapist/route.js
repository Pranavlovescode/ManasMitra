...
export async function GET(req) {
  try {
    ...
    // Find the patient
    const patient = await Patient.findOne({ clerkId: userId }).populate(
      "medicalInfo.assignedTherapist",
    ...
    console.log(
      "Debug - Assigned Therapist:",
      patient?.medicalInfo?.assignedTherapist
    );
    ...
    if (assignedTherapist?._id) {
      try {
        const therapistDoc = await (
          await import("@/models/Therapist")
        ).default.findOne({ userId: assignedTherapist._id });
        therapistDocId = therapistDoc?._id || null;
      } catch (e) {
        console.warn(
          "Unable to resolve Therapist doc for assigned user:",
          e?.message
        );
      }
    }
